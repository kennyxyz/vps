...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/kennyxyz/vps/master/ocserv.conf"
...
Service_ocserv(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/kennyxyz/vps/master/ocserv_debian -O /etc/init.d/ocserv; then
...
Installation_dependency(){
	...
	if [[ ${release} = "centos" ]]; then
		...
		if [[ $? = 0 ]]; then
			...
			wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/kennyxyz/vps/master/us.sources.list"
			apt-get update
			apt-get install vim net-tools pkg-config build-essential libgnutls28-dev libwrap0-dev liblz4-dev libseccomp-dev libreadline-dev libnl-nf-3-dev libev-dev gnutls-bin -y
